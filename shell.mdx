...
- it recieves the input => run programs => send the output
...
- to interact with the shell we use a program called the termianl
...
## termianl (termianl emulator) VS TTY (teletype) VS console VS shell VS prompt VS command line VS CLI (command line interface) VS GUI (graphical user interface)

- termianl: tty = input/output environment
...
- when you press enter
  ...
  - runs it
  - send the output back to the termianl
- you can use the termianl without the shell and using the python interpreter instead
- programs you can use inside the termianl
  - shell
  - python interpreter
- you can use the shell without the termianl by using shell scripting
...
- you can use the shell without the termianl
  - by writing shell commands into a file and arrage for your computer to run the shell program on that file (shell scripting)
- you can use the termianl without the shell
